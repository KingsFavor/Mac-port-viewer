...
Homebrew 로 업데이트해 새 `.app` 이 설치되면, 실행 중이던(옛 버전) 앱이 **유휴 상태에서 자동으로 새 버전으로 재실행**됩니다. 메뉴바 전용 앱이라 창 깜빡임 없이 아이콘만 잠깐 갱신됩니다. 원치 않으면 메뉴의 **"업데이트 설치되면 자동 재실행"** 을 끄세요 — 그러면 대신 **"재실행하여 X 적용"** 항목이 떠서 직접 누를 수 있습니다.

> 자동 재실행은 메뉴가 닫혀 있을 때만 동작하며, 앱이 스스로 파일을 교체하지는 않습니다(설치는 Homebrew/DMG 담당). 직접 내려받았다면 최신 `.dmg` 로 교체하세요.
