...
- 각 프로세스의 **시작 시각** 표시 (상대 시간, 하위 메뉴에 정확한 시각)
- **종료(SIGTERM)** / **강제 종료(SIGKILL)** 로 프로세스 정리
- 포트 번호·PID 클립보드 복사
- "시스템·백그라운드 포트 표시" 토글 (기본은 숨김)
- **업데이트 확인** — 새 버전을 자동으로 알려주고, 원할 때 직접 확인도 가능
...
- **업데이트 명령 복사 (brew)** — `brew update && brew upgrade --cask port-killer` 를 클립보드에 복사합니다. (`brew update` 를 함께 실행해야 tap 캐시가 갱신되어 "이미 최신 버전" 오판 없이 새 버전으로 올라갑니다.)

직접 확인하고 싶다면 메뉴의 **"업데이트 확인"** 을 누르세요. 결과(최신 / 새 버전 / 재실행 필요)를 창으로 확실하게 알려줍니다.

Homebrew 로 업데이트해 새 `.app` 이 설치되면, **실행 중이던 앱은 여전히 옛 버전**입니다. 이때는 메뉴에 **"재실행하여 X 적용"** 이 나타나며, 누르면 새 버전으로 다시 시작합니다. (앱이 스스로 파일을 교체하지는 않습니다. 직접 내려받았다면 최신 `.dmg` 로 교체하세요.)
