...
2. `Developer ID Application: 이름 (팀ID)` 항목 왼쪽 **▶ 삼각형을 펼쳐** 그 아래 **개인 키가 있는지 확인**
   - **▶ 삼각형 자체가 없으면 = 이 Mac 에 개인 키가 없음.** 개인 키는 인증서를 처음 발급할 때 CSR 을 만든 Mac 에만 있고 Apple 서버에도 없어 재다운로드 불가입니다. → **아래 "개인 키가 없을 때" 로.**

#### 개인 키가 없을 때 (▶ 삼각형이 없을 때)
**A. 원래 Mac 이 있으면**: 그 인증서를 처음 발급받은 Mac 의 키체인에 개인 키가 있습니다. 거기서 위 2~5번대로 `.p12` 를 내보내세요.

**B. 원래 Mac 이 없으면 → 이 Mac 에서 재발급** (개인 키를 새로 생성):
1. **키체인 접근 → 인증서 지원 → 인증 기관에서 인증서 요청…** → 이메일 입력, **"디스크에 저장"** + **"본인이 키 쌍 정보 지정"** → CSR 저장 (이때 개인 키가 이 Mac 에 생성됨)
2. <https://developer.apple.com/account/resources/certificates/list> → **＋** → **Developer ID Application** → CSR 업로드 → `.cer` 다운로드 → 더블클릭 설치
   - ⚠️ Developer ID 인증서는 **Account Holder** 권한만 발급 가능
3. 이제 `security find-identity -v -p codesigning` 에 나타나며, "내 인증서" 에 ▶ 가 생깁니다 → 아래 3~5번으로 `.p12` 내보내기
